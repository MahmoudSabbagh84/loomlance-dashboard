...
-- Owner decision: the demo auth user (d3a70000-0000-4000-8000-000000000001) is excluded from
-- every metric here — total, active7d, active30d, and signupsByWeek.
...
as $$
  select jsonb_build_object(
    'total',     (select count(*) from auth.users where id <> 'd3a70000-0000-4000-8000-000000000001'),
    'active7d',  (select count(*) from auth.users where id <> 'd3a70000-0000-4000-8000-000000000001' and last_sign_in_at >= now() - interval '7 days'),
    'active30d', (select count(*) from auth.users where id <> 'd3a70000-0000-4000-8000-000000000001' and last_sign_in_at >= now() - interval '30 days'),
    'signupsByWeek', (
      select coalesce(
      ...
      left join (
        ...
        from auth.users
        where id <> 'd3a70000-0000-4000-8000-000000000001'
        group by 1
      ) c using (week_start)
    )
  );
