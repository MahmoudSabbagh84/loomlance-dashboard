-- LOO-35: Exclude partially_paid invoices from online payability on the public invoice page.
-- A partially_paid invoice already has a recorded payment; offering card/PayPal again would
-- let the client re-pay the full original total and be overcharged. Such invoices fall through
-- to payment_instructions instead. Only the two payability predicates change; everything else
-- in this function is reproduced verbatim from 20260625100000_payflow_connection_derived.sql.
...
begin
  ...
  return jsonb_build_object(
    ...
    -- partially_paid excluded: client has already paid; re-offering card would overcharge them.
    'can_pay', (inv.stripe_connect_account_id is not null and inv.status not in ('paid','void','partially_paid')),
    -- PayPal link is shown whenever it is set and the invoice is still payable.
    -- partially_paid excluded: same overcharge risk as card.
    'paypal_link', case when inv.status not in ('paid','void','partially_paid') then inv.paypal_link else null end,
