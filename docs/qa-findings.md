...
### F6 — No consistent back / breadcrumb navigation; users must re-select from the sidebar to go back  [P2] ✅ FIXED
...
- **Resolution (implemented 2026-06-21, Phase 8):** added a reusable `src/components/ui/Breadcrumbs.jsx` (parent-route based: `Parent / Current`, parent clickable) on every detail page — `ClientDetailPage` (`Clients / …`), `ProjectDetailPage` (`Projects / …`), `ContractDetailPage` (`Contracts / …`), `InvoiceDetailPage` (`Invoices / …`), and `RecurringInvoicesPage` (`Invoices / Recurring`, replacing its ad-hoc back button). Parent-route (not browser-history) for predictability. Render test added; 81/81. Commit pending.

### F7 — Topbar timer is just a clock icon; rework into an explicit timer + controls + tracking-state animation  [P3 · enhancement] ✅ FIXED
...
- **Resolution (implemented 2026-06-21):** plan `docs/superpowers/plans/2026-06-21-f7-timer-rework.md`. Built **true pause (one entry)**: a running session can Pause (freezes, paused time excluded) ⇄ Resume, then **Commit (✓)** registers the active time toward the project, or **Discard (✕, with ConfirmDialog)**. Added `time_entries.paused_at` + `paused_seconds`, `activeSeconds()` helper (tested), pause/resume API + commit math, and a **breathing red dot** (amber when paused; `prefers-reduced-motion` safe). Verified live via MCP (pause→resume→commit = 20m; commit-while-paused = 10m). Commit `df7cb3a`.
...
### F9 — Client detail: "Contact" section (Overview) vs empty "Contacts" tab is confusing — rework or remove  [P3 · brainstorming] ✅ FIXED
...
- **Resolution (implemented 2026-06-21, Phase 10):** **folded into Overview** (chosen approach). Removed the "Contacts" tab; moved its multi-contact management into a new `PeopleCard` (`src/features/clients/PeopleCard.jsx`) shown on the Overview tab; renamed Overview's "Contact" card → "**Details**" to kill the name clash. `client_contacts` (add/edit/delete/set-primary) is preserved — only its location changed; nothing else consumed it. `ContactsTab.jsx` deleted. eslint clean, 81/81 tests, build OK. Commit pending.

### F10 — Client detail → Activity tab is a stale stub: "Activity timeline appears here. (Phase 2.)"  [P2] ✅ FIXED
...
- **Resolution (two stages):** (1) Track A replaced the misleading "(Phase 2.)" copy with an honest empty state. (2) **Phase 8 (2026-06-21) built the real timeline:** `ActivityTab` now aggregates the client's **invoices** (issue date + status), **contracts** (created), and **expenses** (logged) into one chronological, iconed timeline (sorted newest-first), with an empty state. Time entries deferred (project-scoped — per scope decision). Commit pending.
...
- **Triage:**
  ...
  - **✅ (c) RESOLVED via Expenses v2 (2026-06-21).** Plan `docs/superpowers/plans/2026-06-21-expenses-v2.md`. Implemented per-project billing with **find-or-append**: a shared `_find_or_create_draft(user, client, project-or-null, currency)` means generating for a project (from time OR expenses) **appends** to that project's existing draft → one unified per-project draft. New `generate_invoice_from_expenses_for_project/_for_client` RPCs; the time RPC now appends too. Client-only expenses bill into a per-client draft. New `ExpensesReadyToBillPanel` (per-project + per-client rows) replaces `GenerateExpenseInvoiceModal` (deleted). Verified live via MCP: time→draft, expenses→**same** draft (3 lines, positions continue), client-only→separate draft, `NO_BILLABLE_EXPENSES` on empty. Commit `40874c0`. (a)+(b) were already fixed in Track A.
...
### F17 — Project form: replace raw color input with preset swatches + custom  [P3 · enhancement] ✅ FIXED
- **Where:** Project create/edit form (`src/features/projects/ProjectFormModal.jsx`).
- **Reported:** "on project creation… instead of the color picker, set around 10 colors the user can choose from, then a 'custom' swatch that opens the color picker."
- **Resolution (2026-06-21):** new reusable `ColorPicker` (`src/components/ui/ColorPicker.jsx`) — 10 preset swatches (palette in `src/lib/colors.js` `PROJECT_COLORS`) with a selected-ring + check, plus a **custom** swatch that opens the native color picker (and shows the chosen custom color). Wired into the project form (controlled via `setValue`); default = first preset. eslint clean, 81/81 tests, build OK. Commit pending.
