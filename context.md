...
> 4. **Phase 5 is fully DONE** — 5a SES email + 5b Stripe pay (test mode) + 5c PayPal link MVP all shipped. PayPal: `profiles.paypal_link` → public "Pay with PayPal" button (`src/lib/paypal.js`), manual mark-paid `method:'paypal'`. (`.env.local` now: `VITE_EMAIL_PROVIDER=ses`, `VITE_PAYMENTS_PROVIDER=stripe`, `VITE_PUBLIC_SITE_URL=http://localhost:4173`; `app_config.mock_payments_enabled=false`.) Next real work: deploy the dashboard + Stripe live-mode go-live; or Phase 6/7/11.
