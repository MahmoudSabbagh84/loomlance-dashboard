...
>
> **Last updated:** 2026-06-22 (machine switch Mac→PC). **Both apps are DEPLOYED on AWS Amplify:** splash `loomlance.com`, dashboard `app.loomlance.com` (Route 53 zone `Z0013195E0B2HJ5WR66W`, AWS acct `183631341841`). Source of truth: `docs/phases.md` + this file + the two repos' `docs/`. **Two repos now** — dashboard (`loomlance-dashboard`) and **splash (`loomlance-splash` ≈ sibling dir; GitHub `loomlance-splah`)**.
>
> ### ⏭️ RESUME HERE (on the PC)
> 1. **`git pull` BOTH repos.** Dashboard: `npm install`, recreate `.env.local` if missing (§0; `VITE_EMAIL_PROVIDER=ses`, `VITE_PAYMENTS_PROVIDER=stripe`, `VITE_PUBLIC_SITE_URL=http://localhost:4173`), `npx vitest run` (**100 green**). **Splash now has a Tailwind build** → `npm install` in the splash too (its build: `npm run build:css` → `app.css`).
> 2. **IN PROGRESS: splash redesign (QA Track A) — `loomlance-splash`.** Aligning the splash to the dashboard's "Slate Pro" violet design system via **Tailwind** (spec: `loomlance-splash/docs/superpowers/specs/2026-06-22-splash-redesign.md`). **Done:** Phase 1 (Tailwind foundation mirroring the dashboard — `tailwind.config.js`, `src/tokens.css`, `src/input.css` → built `app.css`; commit `8f8eebc`) + Phase 2 (home `index.html` rebuilt: dark violet hero, light feature cards = real shipped features, dark CTA/footer; commit `5cbf6af`). **NEXT — Phases 3–5:** rebuild `pricing.html`, `contact.html`, then `signup.html` (+**F3**: drop username, fix email autocomplete), then `signin.html` (as the **unified sign-in / F2** + remember-me) and **F5** (point dashboard logout there). Each rebuilt page swaps `styles.css`→`app.css` + Outfit font; **rebuild `app.css` (`npm run build:css`) before committing any page** since Tailwind tree-shakes by scanned classes. ⚠️ Mid-transition: home is new, other pages still old orange `styles.css` — `loomlance.com` shows that mix until Phases 3–5 land.
> 3. **Splash QA findings + triage:** `loomlance-splash/docs/qa-findings.md` (F1–F8 + N1, tracks A–D). Track A in progress; **C** (F4 branded auth email — needs SES SMTP creds), **D** (F8 anti-fraud, F7/N1 admin console) pending.
> 4. **DONE & deployed:** Phase 5 (SES email + Stripe Connect pay + PayPal) ; **F6 Stripe subscription billing** (test mode — `create-subscription-checkout`/`stripe-subscription-webhook`/`create-billing-portal` + Subscription tab; spec `docs/superpowers/specs/2026-06-22-pricing-tiers-and-subscriptions.md`); **pricing finalized** (Solo free / Freelancer $19 / Studio $49 / Agency soon) & reconciled across splash+tier.js; **security: DB tier enforcement** (project limit + `enforce_tier_feature` triggers + `protect_billing_columns` fixing a self-upgrade hole — migrations `20260622*`); autosave everywhere (Phase 12).
> 5. **Outstanding go-live chores (USER):** Stripe **live cutover** (live products/prices/webhooks + swap Supabase secrets to `sk_live_`, re-connect Connect in live) — deferred; delete the **root AWS access key** pasted during setup + swap the `loomlance` CLI profile off root; **Supabase Auth URL config** (Site URL `https://app.loomlance.com` + redirect URLs) for confirmation/reset emails.
> 6. **superpowers** plugin not installed — brainstorm/plan run manually (artifacts in `docs/superpowers/`). The **Stripe** plugin IS installed.
